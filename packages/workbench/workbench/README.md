...
- **An idea's invisibility is enforced in the dependency set, not in the transcript** — `renderSkeletonIndex` filters by vantage, so an idea never reaches a model request *through the tool*. What was SAID in an idea area is an ordinary `user/message` and stays in `session.deriveMessages()`, which every later request carries whole. Closing this is not a plugin change: an invariant requires a request's messages to equal the durable derivation exactly, failing with `log-reconstruction desync` otherwise, so per-module scoping needs either a real sub-session per module or a logged scoping projection.
